...
// Set the correct maximum duty resolution for all targets (ESP32-C3 and most ESP chips only support up to 14 bits)
#define LEDC_DUTY_RES_MAX LEDC_TIMER_14_BIT
...
static hal_err_t hal_pwm_configure_timer(int index, uint32_t freq_hz, unsigned duty_resolution_bits) {
    ...
    if (duty_resolution_bits < LEDC_TIMER_10_BIT || duty_resolution_bits > LEDC_DUTY_RES_MAX) {
...
hal_err_t hal_pwm_set_duty(hal_gpio_t gpio, uint32_t duty) { return hal_pwm_set_duty_fading(gpio, duty, 0); }
