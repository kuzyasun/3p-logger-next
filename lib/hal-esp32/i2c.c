...
hal_err_t hal_i2c_cmd_master_start(hal_i2c_cmd_t *cmd) { return i2c_master_start(cmd->handle); }

hal_err_t hal_i2c_cmd_master_stop(hal_i2c_cmd_t *cmd) { return i2c_master_stop(cmd->handle); }
...
hal_err_t hal_i2c_cmd_master_exec(hal_i2c_bus_t bus, hal_i2c_cmd_t *cmd) {
    ...
    return i2c_master_cmd_begin(bus, cmd->handle, 100 / portTICK_PERIOD_MS);
}