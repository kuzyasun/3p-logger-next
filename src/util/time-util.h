...
inline time_ticks_t time_ticks_now(void) { return xTaskGetTickCount(); }

inline time_millis_t time_millis_now(void) { return xTaskGetTickCount() * portTICK_PERIOD_MS; }

inline void time_millis_delay(unsigned ms) { vTaskDelay(ms / portTICK_PERIOD_MS); }
...
inline time_micros_t time_micros_now(void) { return hal_time_micros_now(); }
